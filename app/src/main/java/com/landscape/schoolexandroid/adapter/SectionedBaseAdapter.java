...
public abstract class SectionedBaseAdapter extends BaseAdapter {
    ...
    private SparseArray<Integer> mPositionSectionCache;
    ...
    public SectionedBaseAdapter() {
        ...
        mPositionSectionCache = new SparseArray<Integer>();
        mCount = -1;
        mSectionCount = -1;
    }
    ...
    public int getPostionForSection(int section) {
        // first try to retrieve values from cache
        Integer cachedSection = mPositionSectionCache.get(section);
        if (cachedSection != null) {
            return cachedSection;
        }
        int position = 1;
        for (int i = 0; i < internalGetSectionCount(); i++) {
            mPositionSectionCache.put(i, position);
            position += (internalGetCountForSection(i) + 1);
        }

        cachedSection = mPositionSectionCache.get(section);
        if (cachedSection != null)
            return cachedSection;
        return -1;
    }
